...
class STRIPS(SearchDomain):
    ...
    def result(self, state, action):
        ...
        # Apply the action by updating the state
        new_state = set(state)  # Create a copy of the current state
        for p in action.neg:      # Remove negative effects
            new_state.discard(p)
        for p in action.pos:      # Add positive effects
            new_state.add(p)
    ...
    def satisfies(self, state, goal):
        # Check if all predicates in the goal are present in the state
