...
class Agent:
    ...
    def solve(self, state):
        ...
        if not visible_food:
            self.wander(sight_range, is_traverse, body, bad_food_list, num_players, other_snake_positions) 
            if self.path:
    ...
    def tree_search(self, depth, head, body, food, bad_food_list):
        ...
        def expand_state(head, body, food, direction, bad_food_list):
            ...
            current_state = (head, body, food)
            next_state = ((next_x, next_y), set(new_body), food)

            current_score = heuristic(current_state)
            next_score = heuristic(next_state)

            if next_score < current_score:  # Se o novo estado é pior
                return None
